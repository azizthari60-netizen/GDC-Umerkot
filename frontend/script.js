...
document.addEventListener('DOMContentLoaded', () => {
    ...
    resultsForm.addEventListener('submit', async (e) => {
        ...
        if (submitButton) {
            submitButton.disabled = true;
            submitButton.textContent = 'مطلوبہ ڈیٹا تلاش ہو رہا ہے...';
        }

        try {
            // اپنے backend کا صحیح endpoint پاس کریں
            ...
            if (res.ok && data.success && data.results && data.results.length > 0) {
                ...
                resultsDisplay.innerHTML = `
                    <div style="margin-top: 20px; padding: 15px; border: 1px solid #ccc; border-radius: 8px; background: #fff;">
                        <h3 style="margin-bottom: 1rem; color: #1e3a8a;">رزلٹ کی تفصیلاٹ</h3>
                        <table style="width: 100%; border-collapse: collapse; text-align: left;">
                            <tr><th style="padding: 8px; border-bottom: 1px solid #ddd;">رول نمبر / CNIC:</th><td style="padding: 8px; border-bottom: 1px solid #ddd;">${student.rollNo || student.cnic || 'N/A'}</td></tr>
                            <tr><th style="padding: 8px; border-bottom: 1px solid #ddd;">نام (Name):</th><td style="padding: 8px; border-bottom: 1px solid #ddd;">${student.name || 'N/A'}</td></tr>
                            <tr><th style="padding: 8px; border-bottom: 1px solid #ddd;">والد کا نام (Father Name):</th><td style="padding: 8px; border-bottom: 1px solid #ddd;">${student.fatherName || 'N/A'}</td></tr>
                            <tr><th style="padding: 8px; border-bottom: 1px solid #ddd;">حاصل کردہ نمبر (Marks):</th><td style="padding: 8px; border-bottom: 1px solid #ddd;">${student.marks ?? 'N/A'}</td></tr>
                            <tr><th style="padding: 8px; border-bottom: 1px solid #ddd;">شعبہ (Applied For):</th><td style="padding: 8px; border-bottom: 1px solid #ddd;">${student.appliedFor || 'N/A'}</td></tr>
                            <tr><th style="padding: 8px; border-bottom: 1px solid #ddd;">مقرر کردہ کلاس (Assigned Class):</th><td style="padding: 8px; border-bottom: 1px solid #ddd; font-weight: bold; color: #2563eb;">${assignedClass}</td></tr>
                        </table>
                    </div>
                `;
            } else {
                resultsDisplay.innerHTML = `<p style="color: red; margin-top: 15px;">${data.message || 'کوئی رزلٹ نہیں ملا۔'}</p>`;
            }
        } catch (error) {
            console.error('Fetch Error:', error);
            resultsDisplay.innerHTML = `<p style="color: red; margin-top: 15px;">سرور کے ساتھ رابطہ نہیں ہو سکا۔ دوبارہ کوشش کریں۔</p>`;
        } finally {
            if (submitButton) {
                submitButton.disabled = false;
                submitButton.textContent = originalText;
...
function determineAssignedClass(student) {
    ...
    return 'First Year'; // ڈیفالٹ فال بیک
}
